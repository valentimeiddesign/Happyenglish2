...
// Bundled fallback images keyed by product slug (DB seed image paths are dev-only).
const SLUG_IMAGES: Record<string, string> = {
  "go-getter-1": imgGoGetter1,
  "go-getter-2": imgGoGetter2,
  "go-getter-3": imgGoGetter3,
  "4minds-a2": imgMindsA2,
};
...
const DEFAULTS: Course[] = [
  { slug: "go-getter-1", title: "Go Getter 1", age: "7-10 років", lessons: 45, description: "Базовий рівень для молодших школярів. Яскраві герої та цікаві історії.", emoji: "📘", level: "A1", image_url: null, payment_link: "https://www.privat24.ua/rd/send_qr/liqpay_static_qr/payment_2748372941.2ad3a7668b74413c8ac7fbde30b89ba15925249514454540b71e6ce00b1e95c2", price: 1200, currency: "UAH" },
  { slug: "go-getter-2", title: "Go Getter 2", age: "8-11 років", lessons: 48, description: "Продовження курсу для поглиблення знань. Більше граматики та лексики.", emoji: "📗", level: "A1+", image_url: null, payment_link: "https://www.privat24.ua/rd/send_qr/liqpay_static_qr/qr_f16798af16494eb7b49baa65a673a3b5", price: 1200, currency: "UAH" },
  { slug: "go-getter-3", title: "Go Getter 3", age: "9-12 років", lessons: 50, description: "Просунутий рівень для впевненого володіння. Готуємось до середньої школи.", emoji: "📕", level: "A2", image_url: null, payment_link: "https://www.privat24.ua/rd/send_qr/liqpay_static_qr/qr_d4acec19e536429aa22dc68fb01f531e", price: 1200, currency: "UAH" },
  { slug: "4minds-a2", title: "4Minds A2", age: "10-14 років", lessons: 52, description: "Комплексний курс для підлітків. Сучасні теми та актуальна лексика.", emoji: "📙", level: "A2", image_url: null, payment_link: "https://www.privat24.ua/rd/send_qr/liqpay_static_qr/qr_d4acec19e536429aa22dc68fb01f531e", price: 1200, currency: "UAH" },
];

function resolveImage(c: Course): string {
  if (c.image_url && /^https?:\/\//.test(c.image_url)) return c.image_url;
  return SLUG_IMAGES[c.slug] ?? imgGoGetter1;
}
...
export function Catalog() {
  ...
  useEffect(() => {
    supabase
      .from("products")
      .select("slug,title,age,lessons,description,emoji,level,image_url,payment_link,price,currency")
      .eq("type", "course")
      .eq("is_active", true)
      .order("sort_order")
      .then(({ data, error }) => {
                  ...
                  <div className="flex items-end justify-between mb-4">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-foreground">
                        {course.price}{course.currency === "UAH" ? "₴" : " " + course.currency}{" "}
                        <span className="text-sm font-normal text-muted-foreground">одноразово</span>
                      </p>
                    </div>
                  </div>
